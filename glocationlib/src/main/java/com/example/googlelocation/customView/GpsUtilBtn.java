 ...
 *
 * <IMPORTANT>handle PermissionUtil, LocationUpdateUtil by yourself</IMPORTANT>
 */
public class GpsUtilBtn extends AppCompatTextView implements IViewActionHandler {
    Context context;
    private String TAG = GpsUtilBtn.class.getSimpleName();
    protected GpsUtil gpsUtil;//have to get an instance
    private IActivityResultObserver activityResultObserver;
    public GpsUtilBtn(Context context, AttributeSet attrs) {
        super(context, attrs);
        initLocatParsing(context);
    }

    public GpsUtilBtn(Context context){
        super(context);
        initLocatParsing(context);
    }
    ...
    //***************** get GpsUtil to execute by yourself ***************************************//

    public GpsUtil getGpsUtil() { return gpsUtil; }
